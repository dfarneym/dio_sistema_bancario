import textwrap

def menu():
    menu = """
    #################### MENU ################
    [d]Depositar
    [s]Sacar
    [e]Extrato
    [nc]Nova conta
    [lc]Listar contas
    [nu]Novo usuário
    [q]Sair    

    =>"""
    return input(textwrap.dedent(menu))

# Passando como referencia por posição. Não aceita se você passar o valor
def depositar(saldo,valor,extrato, /):
    if valor > 0:
            saldo+= valor  
            extrato+= f'Deposito: R$ {valor:.2f}\n' 
            print("O depósito feito com sucesso!") 

    else:
        print("A operação falhou! Digite um valor positivo! ")
            
    return saldo, extrato

# Passando uma referencia nomeada, por chave
def sacar(*, saldo,valor,extrato,limite,numero_saques,limite_saques):
    
    passou_do_saldo = valor > saldo
    passou_do_limite = valor > limite
    excedeu_saques = numero_saques >= limite_saques

    if passou_do_saldo:
        print("Sem saldo!")
        
        pergunta = input("Você deseja fazer um empréstimo? Sim(s),Não(n)")
        novo_saldo=float(input("Digite o valor do empréstimo: "))
        if pergunta == "s":
            saldo+=novo_saldo
            emprestimo = saldo
            extrato+= f'Empréstimo: R$ {emprestimo:.2f}\n'  
            print("Emprestimo ralizado com sucesso!")

    elif passou_do_limite:
        print("Saque passou do limite!")   
   

    elif excedeu_saques:
            print("O número de saque excedeu!")
    
    elif valor > 0:
            saldo -= valor
            extrato += f'Saque:\t\tR$ {valor:.2f}\n'
            numero_saques += 1 
            print("Saque realizado com sucesso!")

    else:
            print("Operação inválida,por favor selecione a operação desejada.")
                
    return saldo, extrato
    

def mostar_extrato(saldo, /, *, extrato):     
        print("\n************** EXTRATO BANCARIO ******************")
        print("Não foram realizadas movimentações." if not extrato else extrato)
        print(f"\nSaldo:\t\tR$ {saldo:.2f}")
        print("****************************************************")

def criar_usuario(usuarios):
    cpf = input("Informe o CPF (somente números): ")
    usuario = filtrar_usuario(cpf, usuarios)

    if usuario:
         print("Jáexist usuario com esse CPF!")
         return
    
    nome = input("Informe o nome completo: ")
    data_nascimento = input("Informe a data de nascimento (dd-mm-aaaa): ")
    endereco = input("Informe o endereço (logradouro, nro - bairro - cidade/sigla estado): ")

    usuarios.append({"nome": nome, "data_nascimento": data_nascimento, "cpf": cpf, "endereco": endereco })
    print("Usuario criado com sucesso!")

def filtrar_usuario(cpf, usuarios):
     usuarios_filtrados = [usuario for usuario in usuarios if usuario["cpf"] == cpf]
     return usuarios_filtrados[0] if usuarios_filtrados else None

def criar_conta(agencia, numero_conta, usuarios):
     cpf = cpf = input("Informe o CPF do usuário: ")
     usuario = filtrar_usuario(cpf, usuarios) 

     if usuario:
          print("Conta foi criada com sucesso!")
          return {"agencia": agencia, "numero_conta": numero_conta, "usuario": usuario}          

def listar_contas(contas):
        for conta in contas:
            linha = f"""\
            Agência:\t{conta['agencia']}
            C/C:\t\t{conta['numero_conta']}
            Titular:\t{conta['usuario']['nome']}
        """
        print("=" * 100)
        print(textwrap.dedent(linha))



def main():
    LIMITE_SAQUES = 3
    AGENCIA = "0001"

    saldo = 0
    limite = 500
    extrato = ""
    numero_saques = 0
    usuarios = []
    contas = []

    while True:

        opcao = menu()

        if opcao == "d":
            
            valor = float((input("Digite o valor do valor: ")))
            
            saldo, extrato = depositar(saldo, valor, extrato)
        
        elif opcao == "s":
       
            valor = float(input("Digite o valor do saque: "))

            saldo, extrato = sacar(
                  saldo=saldo,
                  valor=valor,
                  extrato=extrato,
                  limite=limite,                  
                  numero_saques=numero_saques,
                  limite_saques=LIMITE_SAQUES,
            )
            

        elif opcao == "e":
             mostar_extrato(saldo, extrato=extrato)

        elif opcao == "nu":
             criar_usuario(usuarios) 
        
        elif opcao == "nc":
             numero_conta = len(contas) + 1
             conta = criar_conta(AGENCIA, numero_conta, usuarios)

             if conta:
                  contas.append(conta)

        elif opcao == "lc":
             listar_contas(contas)
                                
        elif opcao == "q":
            break

        else:
            print("Operação inválida,por favor selecione a operação desejada.")


main()
